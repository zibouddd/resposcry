...
#[command(
    name = "reposcry",
    version,
    about = "Graph and review commands for RepoScry"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
fn refactor_tool_value(
    ...
    match action.to_lowercase().as_str() {
        ...
        "split-file" | "split_file" => {
            ...
            let modules = split_file_modules(&graph, target);
            let public_exports_to_preserve = graph
                .nodes
                .values()
            ...
            Ok(json!({
                ...
                "impacted_files": walk_reverse_imports(&graph, &[target.to_string()], 3),
                "public_exports_to_preserve": public_exports_to_preserve,
                "suggested_modules": modules,
                "suggested_strategy": [
                    "Move one suggested module at a time, starting with private helpers and low fan-in clusters.",
                    "Preserve current public exports at the original file boundary until callers are migrated.",
                    "Use the suggested module names as a starting point, then validate imports and tests after each extraction."
                ]
            }))
        }
...
fn split_file_modules(graph: &CodeGraph, target: &str) -> Vec<serde_json::Value> {
    let symbols = graph
        .nodes
        .values()
        .filter(|node| node.kind != NodeKind::File && node.file_path.as_deref() == Some(target))
        .collect::<Vec<_>>();
    let symbol_ids = symbols.iter().map(|node| node.id).collect::<HashSet<_>>();
    let mut adjacency: HashMap<u64, HashSet<u64>> = symbols
        .iter()
    ...
    let mut prefix_groups: BTreeMap<String, Vec<u64>> = BTreeMap::new();
    for node in &symbols {
        if let Some(prefix) = symbol_prefix(&node.name) {
            prefix_groups.entry(prefix).or_default().push(node.id);
        }
    }
    for ids in prefix_groups.values().filter(|ids| ids.len() > 1) {
        let first = ids[0];
        for other in ids.iter().copied().skip(1) {
            adjacency.entry(first).or_default().insert(other);
            adjacency.entry(other).or_default().insert(first);
        }
    }
    ...
    for start in symbols.iter().map(|node| node.id) {
        ...
        let mut component_nodes = component_ids
            .iter()
            .filter_map(|id| graph.get_node(*id))
            .collect::<Vec<_>>();
        component_nodes.sort_by_key(|node| {
            (
        ...
        let component_set = component_ids.iter().copied().collect::<HashSet<_>>();
        let internal_call_edges = graph
            .edges
            .iter()
            .filter(|edge| {
                edge.kind == EdgeKind::Calls
                    && component_set.contains(&edge.source_id)
                    && component_set.contains(&edge.target_id)
            })
            .count();
        let external_callers = component_nodes
            .iter()
            .flat_map(|node| incoming_callers(graph, node.id))
            .filter(|caller| !component_set.contains(&caller.id))
            .filter(|caller| caller.file_path.as_deref() != Some(target))
            .collect::<Vec<_>>();
        let external_callees = component_nodes
            .iter()
            .flat_map(|node| outgoing_callees(graph, node.id))
            .filter(|callee| !component_set.contains(&callee.id))
            .collect::<Vec<_>>();
        let exports_to_preserve = component_nodes
            .iter()
            .filter(|node| is_public_api_node(node))
            .map(|node| node.name.clone())
            .collect::<Vec<_>>();
        let private_helpers_first = component_nodes
            .iter()
            .filter(|node| !is_public_api_node(node))
            .filter(|node| {
                incoming_callers(graph, node.id)
                    .into_iter()
                    .all(|caller| caller.file_path.as_deref() == Some(target))
            })
            .map(|node| node.name.clone())
            .collect::<Vec<_>>();
        let confidence = if component_nodes.len() >= 3 && internal_call_edges >= external_callers.len()
        {
            "high"
        } else if component_nodes.len() >= 2 || external_callers.is_empty() {
            "medium"
        } else {
            "low"
        };
        modules.push(json!({
            "module_name": suggested_split_module_name(target, &component_nodes, component_index),
            "confidence": confidence,
            "symbol_count": component_nodes.len(),
            "internal_call_edges": internal_call_edges,
            "symbols": component_nodes.iter().map(|node| NodeSummary::from(*node)).collect::<Vec<_>>(),
            "exports_to_preserve": exports_to_preserve,
            "safe_first_moves": private_helpers_first,
            "external_callers": summarize_unique_nodes(external_callers),
            "external_dependencies": summarize_unique_nodes(external_callees),
        }));
    }

    modules.sort_by(|left, right| {
        let left_conf = split_confidence_rank(
            left.get("confidence").and_then(Value::as_str).unwrap_or("low"),
        );
        let right_conf = split_confidence_rank(
            right.get("confidence").and_then(Value::as_str).unwrap_or("low"),
        );
        right_conf
            .cmp(&left_conf)
            .then_with(|| {
                right
                    ...
                    .and_then(Value::as_u64)
                    .unwrap_or(0)
                    .cmp(&left.get("symbol_count").and_then(Value::as_u64).unwrap_or(0))
            })
            .then_with(|| {
                left.get("module_name")
                    .and_then(Value::as_str)
                    .unwrap_or("")
                    .cmp(right.get("module_name").and_then(Value::as_str).unwrap_or(""))
            })
    });
...
fn summarize_unique_nodes(nodes: Vec<&GraphNode>) -> Vec<NodeSummary> {
    let mut unique = BTreeMap::new();
    for node in nodes {
        unique.entry(node.id).or_insert_with(|| NodeSummary::from(node));
    }
    unique.into_values().collect()
}

fn split_confidence_rank(confidence: &str) -> u8 {
    match confidence {
        "high" => 3,
        "medium" => 2,
        _ => 1,
...
fn symbol_prefix(name: &str) -> Option<String> {
    let prefix = name
        .split(|ch: char| !ch.is_alphanumeric())
        .find(|part| !part.is_empty())?
        .to_lowercase();
    match prefix.as_str() {
        "get" | "set" | "test" | "new" | "main" => None,
        _ => Some(prefix),
    }
}

fn suggested_split_module_name(target: &str, nodes: &[&GraphNode], index: usize) -> String {
    let dominant_prefix = {
        ...
        counts
            ...
            .and_then(|(prefix, count)| (count > 1).then_some(prefix))
    };
    if let Some(prefix) = dominant_prefix {
        return prefix;
    }
    if let Some(first_prefix) = nodes.first().and_then(|node| symbol_prefix(&node.name)) {
        return first_prefix;
    }
    format!("{}_part_{}", file_stem(target), index)
}
...
mod tests {
    ...
    fn temp_repo_dir() -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("reposcry-test-{}", unique));
    ...
    #[test]
    fn split_file_groups_symbols_into_module_candidates() {
        let mut graph = CodeGraph::new();
        let src = file_node(&mut graph, "src/orders.rs");
        let tests = file_node(&mut graph, "tests/orders_test.rs");
        let process_order =
            symbol_node(&mut graph, NodeKind::Function, "src/orders.rs", "process_order");
        let validate_order =
            symbol_node(&mut graph, NodeKind::Function, "src/orders.rs", "validate_order");
        let persist_order =
            symbol_node(&mut graph, NodeKind::Function, "src/orders.rs", "persist_order");
        let build_invoice =
            symbol_node(&mut graph, NodeKind::Function, "src/orders.rs", "build_invoice");
        let invoice_total =
            symbol_node(&mut graph, NodeKind::Function, "src/orders.rs", "invoice_total");
        let test_symbol =
            symbol_node(&mut graph, NodeKind::Test, "tests/orders_test.rs", "test_process_order");

        graph.nodes.get_mut(&process_order).unwrap().start_line = Some(10);
        graph.nodes.get_mut(&process_order).unwrap().visibility = Some("pub".to_string());
        graph.nodes.get_mut(&validate_order).unwrap().start_line = Some(20);
        graph.nodes.get_mut(&persist_order).unwrap().start_line = Some(30);
        graph.nodes.get_mut(&build_invoice).unwrap().start_line = Some(40);
        graph.nodes.get_mut(&invoice_total).unwrap().start_line = Some(50);
        graph.nodes.get_mut(&test_symbol).unwrap().start_line = Some(10);

        graph.add_edge(src, process_order, EdgeKind::Contains);
        graph.add_edge(src, validate_order, EdgeKind::Contains);
        graph.add_edge(src, persist_order, EdgeKind::Contains);
        graph.add_edge(src, build_invoice, EdgeKind::Contains);
        graph.add_edge(src, invoice_total, EdgeKind::Contains);
        graph.add_edge(tests, test_symbol, EdgeKind::Contains);

        graph.add_edge(process_order, validate_order, EdgeKind::Calls);
        graph.add_edge(process_order, persist_order, EdgeKind::Calls);
        graph.add_edge(build_invoice, invoice_total, EdgeKind::Calls);
        graph.add_edge(test_symbol, process_order, EdgeKind::Calls);

        let modules = split_file_modules(&graph, "src/orders.rs");
        assert_eq!(modules.len(), 2);

        let order_module = modules
            .iter()
            .find(|module| module["module_name"] == "process")
            .unwrap();
        assert_eq!(order_module["confidence"], "high");
        assert!(order_module["symbol_count"].as_u64().unwrap() >= 3);
        assert!(order_module["exports_to_preserve"]
            .as_array()
            .unwrap()
            .iter()
            .any(|value| value == "process_order"));
        assert!(order_module["external_callers"]
            .as_array()
            .unwrap()
            .iter()
            .any(|value| value["name"] == "test_process_order"));

        let invoice_module = modules
            .iter()
            .find(|module| module["module_name"] == "build")
            .unwrap();
        assert_eq!(invoice_module["confidence"], "medium");
        assert_eq!(invoice_module["symbol_count"], 2);
    }
